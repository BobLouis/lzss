...
def check_dont_care(input_file):
    # Open the file
    ...
    if len(bits) % 8 != 0:
        raise ValueError(f"Total number of bits should be a multiple of 8, but got {len(bits)} bits")

    # Initialize list to store 8-bit chunks
    data_list = []

    # Read every 8 bits
    for i in range(0, len(bits), 8):
        # Convert the 8-bit binary number to a list of ints
        data_list.append([int(bit) for bit in bits[i:i+8]])

    # Convert to 2D NumPy array
    data_2d = np.array(data_list)

    # Now data_2d is a 2D NumPy array where each row is the bits of each 8-bit binary number
    # print(data_2d)
    # print(data_2d.shape)
    data_2d = data_2d[:, ::-1]
...
def check_care(processed_file, original_file):
    ...
    # assuming pdata and odata are numpy arrays of the same shape
    # print(process_data.shape)
    # print(origin_data.shape)
    # print(origin_data)
    # print(process_data)
    # assuming process_data and origin_data are numpy arrays of the same shape
    differ_indices = np.where(process_data != origin_data)

    # check if there are any differing indices
    error = 0
    for i in range (process_data.shape[0]):
        if(np.array_equal(process_data[i], origin_data[i]) == False):
            print("process_data", i, process_data[i])
            print("origin_data", i,  origin_data[i])
            error += 1

    # print(origin_data[143])
...
if __name__ == '__main__':
    for i in range(1, 6):
        origin_file = '../lz77/input_binary/binary_BLOCK_'+str(i)+'.txt'
        decompressed_file = '../lz77/decompressed/reconstruct_BLOCK_'+str(i)+'.txt'
        check_care(decompressed_file,origin_file)
        check_dont_care(decompressed_file)